    ...
    on<GetCurrentWeather>((event, emit) async {
      ...
      if (event.city != null) {
        var currentWeatherData = await apiHelper!.getCurrentWeather(
            url:
                "https://api.openweathermap.org/data/2.5/weather?q=${event.city}&appid=5d31bddb440d5e3d481a66d202c500b8");
        var hourlyWeatherData = await apiHelper!.getHourlyWeather(
            url:
                "https://api.openweathermap.org/data/2.5/forecast?q=${event.city}&appid=5d31bddb440d5e3d481a66d202c500b8");
        ...
        var hourlyWeatherData = await apiHelper!.getHourlyWeather(
            url:
                "https://api.openweathermap.org/data/2.5/forecast?lat=${event.lat}&lon=${event.lon}&appid=5d31bddb440d5e3d481a66d202c500b8");
        if (currentWeatherData != null && hourlyWeatherData != null) {
          emit(LoadedState(
              currentWeather: currentWeatherData,
              hourlyWeather: hourlyWeatherData));
